...
from data_provider.stage2_chebi_dm_regression import Stage2CheBIDM
from model_graph_token_regression_v2.QA_stage2 import Blip2Stage2
...
# alignment LLM vocabulary mixer
def main(args):
    ...
    #     print(f"loaded vicuna model from {args.finetune_path}")
    ...
    if args.opt_model.find('llama') >= 0 or args.opt_model.find('vicuna') >= 0:
        tokenizer = model.blip2opt.llm_tokenizer
    else:
        raise NotImplementedError
    # data

    dm = Stage2CheBIDM(args.mode, args.num_workers, args.batch_size, args.root, args.text_max_len, tokenizer,
                       args)
    callbacks = []
    ...
    if len(args.devices.split(',')) > 1:
        if args.strategy_name == 'fsdp':
            ...
            strategy = MyDDPStrategy(find_unused_parameters=True)
    else:
        # strategy = 'auto'
        strategy = MyDDPStrategy(find_unused_parameters=True)
...
def get_args():
    ...
    parser.add_argument('--filename', type=str, default="qm9_regression_16")
